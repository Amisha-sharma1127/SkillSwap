...
function Register() {
  ...
  return (
    <div style={styles.wrapper}>
      <h1 style={styles.brand}>Skill<span style={{ color: "#2FA88F" }}>Swap</span></h1>
      <p style={styles.subtitle}>Create your account — start with 5 free credits.</p>
      <form onSubmit={handleSubmit}>
        <label style={styles.label}>Full Name</label>
        <input style={styles.input} value={name} onChange={(e) => setName(e.target.value)} placeholder="Amisha Sharma" />
        ...
        <input style={styles.input} type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="At least 6 characters" />
        <button style={styles.button} type="submit">Create Account</button>
      </form>
      {error && <div style={styles.error}>{error}</div>}
      <p style={styles.switchText}>Already have an account? <Link to="/login" style={styles.link}>Log in</Link></p>
    </div>
  );
}
...
const styles = {
  ...
  button: { width: "100%", background: "#1B2A4A", color: "#fff", border: "none", padding: 12, borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: "pointer", marginTop: 20 },
